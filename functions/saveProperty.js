  ...
  try {
    ...
    if (!title || !image || !description) {
      return {
        statusCode: 400,
        headers: { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" },
        body: JSON.stringify({ error: "Missing required property fields (title, image, description)." })
      };
    }

    // Insert the new property into the 'properties' table
    const queryText = `
      INSERT INTO properties(title, image, description, created_at)
      VALUES($1, $2, $3, NOW())
      RETURNING id, title, image, description, created_at;
    `;
    const result = await client.query(queryText, [title, image, description]);
